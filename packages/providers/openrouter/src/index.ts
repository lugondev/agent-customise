...
// OpenRouter adapter with optional mock mode
export class OpenRouterAdapter implements ProviderAdapter {
	...
	async chat(input: ChatInput): Promise<ChatResult> {
		...
		const model = (input as { model?: string }).model || process.env.OPENROUTER_DEFAULT_MODEL || "openai/gpt-4o";
		const referer = process.env.OPENROUTER_REFERER;
		const title = process.env.OPENROUTER_TITLE || "Agent Orchestrator";
